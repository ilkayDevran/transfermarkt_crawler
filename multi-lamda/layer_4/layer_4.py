...
def start(URL, league_href):
    ...
    # set league info
    set_league_infos(soup_of_league_page)
...
def set_league_infos(soupOfLeaguePage):
    ...
    try:
        leagueObj.name = soupOfLeaguePage.find('h1', attrs = {'itemprop' : 'name'}).get_text(strip=True)
    except:
        pass
...
def insert_league_data_into_dataBase(name, country, type, totalValue):
    asd
